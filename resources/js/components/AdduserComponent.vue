        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="form-group row mb-4">
                    ...
                    <div class="col-sm-12 col-md-7">
                        <select class="form-control" v-model="company" v-bind:class="{'is-invalid': errors.company}">
                            <option v-for="company in companies" v-bind:value="company.cid">{{ company.com_name }}</option>
                        </select>
                        <div class="invalid-feedback" v-if="errors.role">
                            <p>{{ errors.role[0] }}</p>
                        </div>
...
export default {
    ...
    methods: {
        ...
        getCompany() {
            axios.get(this.$parent.MakeUrl('admin/users/companies')).then((res) => {
        ...
        addUser() {
            ...
            _this.message = '';
            _this.loading = true;
            axios.post(this.$parent.MakeUrl('admin/users'), {'name': this.name, 'role': this.role, 'email': this.email, 'current_password': this.current_password, 'password': this.password, 'password_confirmation': this.password_confirmation, 'cid': this.company}).then((res) => {
                _this.loading = false;
                _this.resetForm();
